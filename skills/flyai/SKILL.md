...
1. **Validate** — before running a command, check that the inputs are reasonable.
   - Dates should not be in the past and should match the expected format per the command's reference doc.
...
2. **Diagnose** — when a command fails or returns unexpected results, check the output for error messages or status codes.
...
4. **Be transparent** — when results appear incomplete or inconsistent with user expectations, present available results and let the user know that results may not match the intended location or criteria. Suggest verifying through other channels if accuracy is critical.
